...
| 1.4 Add minimal lint baseline (resolved 2026-07-21) | Root manifest/config and workspace source globs | ESLint 9 flat config now provides a cross-workspace lint command with documented dependencies. | Resolved; the explicit-`any` sorter cleanup remains isolated in task 2.3. | `pnpm run lint`, full typecheck/build. | Runtime and visuals unchanged; generated files excluded intentionally, not rewritten. |
